...
extension Certificate.Test.Unit {
  ...
  @Test func `range replaceable collection conformance`() throws {
    ...
    ext.replaceSubrange(
      ...
      with: [
        .init(
          method: .ocspServer, location: .uniformResourceIdentifier("http://example.com/ocsp/a")),
        .init(
          method: .ocspServer, location: .uniformResourceIdentifier("http://example.com/ocsp/b")),
      ]
    )
      ...
      Array(ext)
        == [
          ...
          .init(
            method: .ocspServer, location: .uniformResourceIdentifier("http://example.com/ocsp/b")),
          .init(
            method: .init(.unknownType([1, 2, 3, 4])), location: .rfc822Name("mail@example.com")),
        ]
    )
  ...
  @Test func `serial number random number generator`() {
    struct StaticNumberGenerator: RandomNumberGenerator {
      ...
      mutating func next() -> UInt64 {
        ...
        precondition(
          numbers.indices.contains(startOffset), "static number generator is out of numbers")
...
extension Certificate.Test.Integration {
  ...
  @Test func `default RSA signature algorithm`() throws {
    ...
    #expect(
      certificate.signatureAlgorithm.description == "SignatureAlgorithm.sha256WithRSAEncryption")
  }

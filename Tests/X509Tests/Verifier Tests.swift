    ...
    private static let localhostLeaf: Certificate = {
      ...
      return try! Certificate(
        ...
        extensions: Certificate.Extensions {
          ...
          KeyUsage(keyCertSign: true)
          AuthorityKeyIdentifier(
            keyIdentifier: try! intermediate1.extensions.subjectKeyIdentifier!.keyIdentifier)
        },
...
extension `Verifier Tests`.Integration {
  ...
  func `supports cross signed root without trouble`() async throws {
    ...
    #expect(
      Array(chain) == [Self.localhostLeaf, Self.intermediate1, Self.ca1CrossSignedByCA2, Self.ca2])
  ...
  func `insane PKI can still build`() async throws {
    ...
    #expect(
      Array(chain) == [Self.insaneLeaf, Self.t3, Self.x2, Self.t2, Self.x1, Self.t1, Self.ca1])
    ...
    #expect(
      log == [
        ...
        .issuerIsAlreadyInTheChain(
          [Self.insaneLeaf, Self.t3, Self.x2, Self.t2, Self.x1], issuer: Self.t2),
        .issuerIsAlreadyInTheChain(
          [Self.insaneLeaf, Self.t3, Self.x2, Self.t2, Self.x1], issuer: Self.t3),
        .searchingForIssuerOfPartialChain([
  ...
  func `trust roots can be non self signed leaves`() async throws {
    ...
    struct IgnoreBasicConstraintsPolicy: VerifierPolicy {
      ...
      func chainMeetsPolicyRequirements(chain: UnverifiedCertificateChain) async
  ...
  func `trust roots can be non self signed intermediates`() async throws {
    ...
    #expect(
      log == [
        .searchingForIssuerOfPartialChain([Self.localhostLeaf]),
        .foundCandidateIssuersOfPartialChainInRootStore(
          [Self.localhostLeaf], issuers: [Self.intermediate1]),
    ...
    let diagnostics: [VerificationDiagnostic] = [
      ...
      .init(
        storage: .issuerHasNotSignedCertificate(
          Self.intermediate1, partialChain: [Self.localhostLeaf])),
      .init(
      ...
      .init(
        storage: .foundValidCertificateChain([Self.localhostLeaf, Self.intermediate1, Self.ca1])),
      .init(storage: .couldNotValidateLeafCertificate(Self.localhostLeaf)),
      .init(
        storage: .issuerIsAlreadyInTheChain([Self.insaneLeaf, Self.t3, Self.x2], issuer: Self.t3)),
    ]

...
extension Certificate.Signature.Test.Unit {
    ...
    func `map private key to supported signature algorithm eddsa`() throws {
        #expect(Set(Certificate.PrivateKey(Fixtures.ed25519Key).supportedSignatureAlgorithms) == Set([.ed25519]))
    }
}
...
extension Certificate.Signature.Test.`Edge Case` {
    ...
    private func hashFunctionMismatchTest(
        ...
        do {
            ...
            #expect(!validCombination, "Unable to create cert combining \(privateKey) and \(signatureAlgorithm)")
        }
    }

...
@testable @_spi(FixedExpiryValidationTime) import Certificates
...
extension Certificate.Test {
  @Suite struct DER {}
}

extension Certificate.Test.DER {
  @Suite struct `Private Key` {}
}
...
extension Certificate.Test.DER {
  ...
  #if canImport(Security)
    @Test func `sec certificate bridge`() throws {
      let certificateData = Data(
        base64Encoded: Self.base64EncodedSampleCert, options: .ignoreUnknownCharacters)!
      ...
      #expect(cert == secCertConvertedToCert)
    }
  #endif

  @Test func `simple decode`() throws {
    let binary = Array(
      Data(base64Encoded: Self.base64EncodedSampleCert, options: .ignoreUnknownCharacters)!)
    let cert = try Certificate(derEncoded: binary)
    ...
    #expect(cert.version == .v3)
    #expect(
      cert.serialNumber.bytes
        == [
          0x0c, 0xab, 0xaa, 0xd1, 0xce, 0xc4, 0xe9, 0x7c, 0xc2, 0x66, 0x58, 0x81, 0xd0, 0x21, 0x38,
          0xf7,
        ]
    )
    #expect(cert.publicKey == Certificate.PublicKey(expectedPublicKey))
    ...
    #expect(expectedNotBefore == cert.notValidBefore)
    #expect(expectedNotAfter == cert.notValidAfter)

    let expectedIssuer = try DistinguishedName([
      RelativeDistinguishedName.Attribute(
        type: .RDNAttributeType.countryName, printableString: "US"),
      RelativeDistinguishedName.Attribute(
    ...
    #expect(cert.issuer == expectedIssuer)
    #expect(cert.subject == expectedSubject)

    #expect(cert.extensions.count == 8)
    #expect(
      try cert.extensions.authorityInformationAccess
        == .init([
          .init(
            method: .ocspServer, location: .uniformResourceIdentifier("http://ocsp.digicert.com"))
        ])
    )
    #expect(
      try cert.extensions.subjectKeyIdentifier
        == .init(keyIdentifier: [
          0xE0, 0x85, 0x48, 0x7D, 0x13, 0xA6, 0xD3, 0x10, 0x19, 0x9F, 0x5C, 0xCB, 0x6B, 0x78, 0x24,
          0x92, 0xF8,
          0xAE, 0x1B, 0xAE,
        ])
    )
    ...
    #expect(cert.signatureAlgorithm == .ecdsaWithSHA384)
  }

  @Test func `matching extensions via extension builder`() throws {
    let binary = Array(
      Data(base64Encoded: Self.base64EncodedSampleCert, options: .ignoreUnknownCharacters)!)
    let cert = try Certificate(derEncoded: binary)
    ...
    let expectedExtensions = try Certificate.Extensions {
      ...
      Certificate.Extension(
        ...
        critical: false,
        value: [
          0x30, 0x39, 0x30, 0x37, 0xA0, 0x35, 0xA0, 0x33, 0x86, 0x31, 0x68, 0x74, 0x74, 0x70, 0x3A,
          0x2F,
          0x2F, 0x63, 0x72, 0x6C, 0x33, 0x2E, 0x64, 0x69, 0x67, 0x69, 0x63, 0x65, 0x72, 0x74, 0x2E,
          0x63,
          0x6F, 0x6D, 0x2F, 0x44, 0x69, 0x67, 0x69, 0x43, 0x65, 0x72, 0x74, 0x47, 0x6C, 0x6F, 0x62,
          0x61,
      ...
      Certificate.Extension(
        ...
        critical: false,
        value: [
          ,
        ]
      )
    ...
    #expect(cert.extensions == expectedExtensions)
  }

  @Test func `subject key identifier hash`() throws {
    let binary = Array(
      Data(base64Encoded: Self.base64EncodedSampleCert, options: .ignoreUnknownCharacters)!)
    let cert = try Certificate(derEncoded: binary)

    #expect(try cert.extensions.subjectKeyIdentifier == SubjectKeyIdentifier(hash: cert.publicKey))
  }

  @Test func `RSA root cert`() throws {
    let binary = Array(
      Data(base64Encoded: Self.base64EncodedRSARootCert, options: .ignoreUnknownCharacters)!)
    let cert = try Certificate(derEncoded: binary)
    #expect(cert.publicKey.isValidSignature(cert.signature, for: cert))
  }

  @Test func `codesign chain`() throws {
    let binaryCerts = Self.codesignCerts.map {
      Array(Data(base64Encoded: $0, options: .ignoreUnknownCharacters)!)
    }
  ...
  @Test func `reencoding does not change the bytes`() throws {
    ...
    try coder.appendConstructedNode(identifier: .sequence) { coder in
      ...
      try coder.serialize(AlgorithmIdentifier.sha256WithRSAEncryptionUsingNil)
      try coder.serialize(ISO_8824.BitString(signature))
    }
    ...
    #expect(reserializedCert == serializedCert)
  }
  ...
  @Test func `RSA key format output is correct`() throws {
    ...
    var encoder = ISO_8825.DER.Serializer()
  ...
  @Test func `incorrect parameter size`() throws {
    ...
    try Certificate(derEncoded: cert)
  }

  @Test func `using weird hash functions`() async throws {
    let now = Date()
    ...
    guard case .validCertificate(let chain) = result else {
      Issue.record("Failed to validate cert")
      return
    }

    #expect(Array(chain) == [parsed, issuer])
  }

  @Test func `parsing big name constraints`() throws {
    let cert = Array(
      Data(
        base64Encoded: """
          ...
          dG9ob2t1LWdha3Vpbi5hYy5qcDASghB0b2hva3UtZ2FrdWluLmpwMBKCEHRvaG9y
    ...
    _ = try decoded.extensions.nameConstraints
  }
}

extension Certificate.Test.DER.`Private Key` {
  @Test func `decode ECDSA P256`() throws {
  ...
  @Test func `decode ECDSA P384`() throws {
  ...
  @Test func `decode ECDSA P521`() throws {

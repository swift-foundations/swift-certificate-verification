...
@Suite struct `DNSNames Tests` {
  ...
  static let fixtures: [(String, String, Bool)] = [
    ...
    // (7 * 31) + 30 + 7 == 254.
    (
      Array(repeating: "example", count: 31).joined(separator: ".") + ".com.au", ".example.com.au",
      false
    ),
    (
      "example.com.au", Array(repeating: "example", count: 31).joined(separator: ".") + ".com.au",
      false
    ),
          ...
          #expect(
            match
              == NameConstraintsPolicy.uriNameMatchesConstraint(
                uriName: uri, constraint: constraint)
          )

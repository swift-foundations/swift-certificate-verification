...
extension ExtendedKeyUsage.Test.`Edge Case` {
  @Test func `init rejects duplicate usages`() {
    #expect(
      throws: Certificate.Error.extension(.duplicateOID(ISO_8824.ObjectIdentifier(.serverAuth)))
    ) {
      try ExtendedKeyUsage([
        .serverAuth,
        .serverAuth,
    ...
    #expect(
      throws: Certificate.Error.extension(.duplicateOID(ISO_8824.ObjectIdentifier(.clientAuth)))
    ) {
      try ExtendedKeyUsage([
        .clientAuth,
        .serverAuth,

...
extension RFC5280Policy.Test.Integration {
  ...
  func `valid v1 certs are accepted`() async throws {
    let roots = CertificateStore([TestPKI.unconstrainedCA])
    let leaf = TestPKI.issueLeaf(
      version: .v1, issuer: .unconstrainedIntermediate, customExtensions: .init())
...
extension RFC5280Policy.Test {
  ...
  static func _selfSignedCertsMustBeMarkedAsCA(_ policyFactory: PolicyFactory) async throws {
    let certsAndValidity = [
      (
        TestPKI.issueSelfSignedCert(basicConstraints: .isCertificateAuthority(maxPathLength: nil)),
        true
      ),
      (
        TestPKI.issueSelfSignedCert(basicConstraints: .isCertificateAuthority(maxPathLength: 0)),
        true
      ),
  ...
  static func _intermediateCAMustBeMarkedCAInBasicConstraints(_ policyFactory: PolicyFactory)
    async throws
  {
    ...
    for badIntermediate in invalidIntermediateCAs {
      ...
      result = await verifier.validate(
        leaf: leaf, intermediates: CertificateStore([v1Intermediate]))
  ...
  static func _pathLengthConstraintsFromIntermediatesAreApplied(_ policyFactory: PolicyFactory)
    async throws
  {
  ...
  static func _pathLengthConstraintsDoesOnlyCountNonSelfIssuedCertificates(
    ...
    let result = await verifier.validate(
      leaf: leaf, intermediates: CertificateStore([intermediate]))
  ...
  static func subtreesOfUnknownTypeAlwaysFail(_ policyFactory: PolicyFactory) async throws {
    let subtrees: [GeneralName] = try [
      ...
      .ediPartyName(
        ISO_8825.`Any`(erasing: ISO_8824.Null(), withIdentifier: GeneralName.ediPartyNameTag)),

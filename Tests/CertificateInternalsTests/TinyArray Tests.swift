...
@Suite struct `TinyArray Tests` {
  ...
  @Suite struct Integration {
    @Test func `throwing init from result`() throws {
      ...
      #expect(
        Array(try _TinyArray([Result<_, Error>.success(1), .success(2), .success(3)])) == [1, 2, 3])
      #expect(
      ...
      #expect(
        Array(
          try _TinyArray([
            Result<_, Error>.success(1), .success(2), .success(3), .success(4), .success(5),
          ]))
          == [1, 2, 3, 4, 5]
      )
      ...
      #expect(throws: (any Error).self) {
        Array(
          try _TinyArray<Int>([.success(1), .success(2), Result.failure(MyError()), .success(4)]))
      }
      #expect(throws: (any Error).self) {
        Array(
          try _TinyArray<Int>([.success(1), .success(2), .success(3), Result.failure(MyError())]))
      }
      #expect(throws: (any Error).self) {
        Array(
          try _TinyArray<Int>([
            .success(1), .success(2), .success(3), .success(4), Result.failure(MyError()),
          ])
        )
      }
    }

...
@usableFromInline
let dnsNameAsciiPeriod = UInt8(ascii: "." as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiAsterisk = UInt8(ascii: "*" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiHyphen = UInt8(ascii: "-" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiLowercaseA = UInt8(ascii: "a" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiLowercaseZ = UInt8(ascii: "z" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiUppercaseA = UInt8(ascii: "A" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiUppercaseZ = UInt8(ascii: "Z" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiZero = UInt8(ascii: "0" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiNine = UInt8(ascii: "9" as Unicode.Scalar)

@usableFromInline
let dnsNameAsciiUnderscore = UInt8(ascii: "_" as Unicode.Scalar)
...
extension NameConstraintsPolicy {
  ...
  package static func dnsNameMatchesConstraint(
    dnsName: String.UTF8View, constraint: String.UTF8View
  ) -> Bool {
    // Before any validation: confirm that these are both valid DNS names.
    guard
      dnsName.isValidDNSName(isConstraint: false) && constraint.isValidDNSName(isConstraint: true)
    else {
      return false
    }
    ...
    if constraint.last == dnsNameAsciiPeriod {
...
extension String.UTF8View {
  ...
  package func isValidDNSName(isConstraint: Bool) -> Bool {
    ...
    // label.
    if bytes.first == dnsNameAsciiAsterisk {
      bytes = bytes.dropFirst()
      guard let next = bytes.popFirst(), next == dnsNameAsciiPeriod else {
    ...
    while bytes.count > 0 {
      ...
      if let nextPeriod = bytes.firstIndex(of: dnsNameAsciiPeriod) {
      ...
      if label.first == dnsNameAsciiHyphen || label.last == dnsNameAsciiHyphen {
        return false
      }
...
struct ReverseDNSLabelSequence: Sequence, Sendable {
  ...
  struct Iterator: IteratorProtocol, Sendable {
    ...
    @inlinable mutating func next() -> String.UTF8View.SubSequence? {
      ...
      guard let periodIndex = base.lastIndex(of: dnsNameAsciiPeriod) else {
...
extension String.UTF8View.SubSequence {
  ...
  package var labelContents: LabelContents {
    ...
    for byte in self {
      switch byte {
      case dnsNameAsciiZero...dnsNameAsciiNine:
        ()

      case dnsNameAsciiLowercaseA...dnsNameAsciiLowercaseZ,
        dnsNameAsciiUppercaseA...dnsNameAsciiUppercaseZ,
        dnsNameAsciiHyphen, dnsNameAsciiUnderscore:
        nonNumerics += 1

  ...
  package init(
    ...
    do {
      self.signature = try Signature(
        signatureAlgorithm: self.signatureAlgorithm, signatureBytes: signature)
    } catch {
...
extension Certificate: ISO_8825.DER.ImplicitlyTaggable {
    ...
    throws(ISO_8824.Error)
  {
    self = try ISO_8825.DER.sequence(rootNode, identifier: identifier) {
      (nodes: inout ISO_8825.Node.Collection.Iterator) throws(ISO_8824.Error) -> Certificate in
...
extension ISO_8825.DER.Serializer {
  @inlinable
  package static func serialized<Element: ISO_8825.DER.Serializable>(element: Element) throws
    -> [UInt8]
  {

...
public enum GeneralName: Hashable, Sendable, ISO_8825.DER.Parseable, ISO_8825.DER.Serializable {
  ...
  public init(derEncoded rootNode: ISO_8825.Node) throws(ISO_8824.Error) {
    ...
    case Self.ipAddressTag:
      self = try .ipAddress(
        ISO_8824.OctetString(derEncoded: rootNode, withIdentifier: Self.ipAddressTag))

    case Self.registeredIDTag:
      self = try .registeredID(
        ISO_8824.ObjectIdentifier(derEncoded: rootNode, withIdentifier: Self.registeredIDTag))
  ...
  public struct OtherName: Hashable, Sendable, ISO_8825.DER.ImplicitlyTaggable {
      ...
      throws(ISO_8824.Error)
    {
      self = try ISO_8825.DER.sequence(rootNode, identifier: identifier) {
        (nodes: inout ISO_8825.Node.Collection.Iterator) throws(ISO_8824.Error) -> OtherName in
        let typeID = try ISO_8824.ObjectIdentifier(derEncoded: &nodes)
        let value = try ISO_8825.DER.optionalExplicitlyTagged(
          &nodes, tagNumber: 0, tagClass: .contextSpecific
        ) {
...
struct GeneralNames: ISO_8825.DER.ImplicitlyTaggable, Sendable {
  ...
  init(derEncoded rootNode: ISO_8825.Node, withIdentifier identifier: ISO_8824.Identifier)
    throws(ISO_8824.Error)
  {
    self.names = try ISO_8825.DER.sequence(
      of: GeneralName.self, identifier: identifier, rootNode: rootNode)
  }
